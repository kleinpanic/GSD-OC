...
const CONDITIONAL: { intent: RegExp; doc: RegExp; stage: Stage }[] = [
  ...
  { intent: /\b(integrat\w*|end-to-end|e2e|cross-phase|wire up|connect (the )?phases?)\b/, doc: /^(workflow:verify-phase|agent:gsd-integration)/, stage: { verb: "integration", skill: "gsd-integration-checker", pos: 82, reason: "cross-phase/E2E — integration check" } },
  // #2 consensus fix: single-doc categories (debug/secure/docs) could never reach CONSENSUS_MIN=2 with only one
  // workflow doc — the retrieval BACKSTOP was dead. Matching the category's AGENT docs too makes consensus reachable
  // (e.g. workflow:debug + agent:gsd-debugger + agent:gsd-debug-session-manager).
  { intent: /\b(bug|debug\w*|flaky|fail\w*|broken|crash\w*|error|reproduce|intermittent|stack ?trace|regression)\b/, doc: /^(workflow:debug|agent:gsd-debug)/, stage: { verb: "debug", skill: "gsd-debug", pos: 55, reason: "bug/failure intent — systematic debug" } },
  ...
  { intent: /\b(secur\w*|vulnerab\w*|threat\w*|exploit\w*|csrf|xss|inject\w*|mitigat\w*|pentest|owasp|cve|auth\w*|oauth|jwt|login|logout|password|credential\w*|sso|saml|rbac|encrypt\w*|decrypt\w*)\b/, doc: /^(workflow:secure|agent:gsd-security)/, stage: { verb: "secure", skill: "gsd-secure-phase", pos: 65, reason: "security-sensitive — threat model" } },
  ...
  { intent: /\b(document\w*|readme|changelog|api docs|docstring)\b/, doc: /^(workflow:docs-update|agent:gsd-doc)/, stage: { verb: "docs", skill: "gsd-docs-update", pos: 88, reason: "documentation request — write/verify docs" } },
];
